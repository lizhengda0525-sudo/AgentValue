...
从 [Releases](https://github.com/lizhengda0525-sudo/AgentValue/releases/latest) 下载已发布版本的 Windows x64 ZIP，完整解压后双击 `AgentVault.exe`。便携版不需要安装 Node.js，也不需要 Codex 或 API Key。使用 GitHub 导入功能需要安装 Git。当前源码版本为 **0.2.0**。
...
- 设置：打开数据目录、导出带 SHA-256 文件校验和的完整备份及 JSON 清单；应用内校验、预览和恢复备份，恢复前自动备份当前库。
...
支持 0.1.0 / Schema 1 备份：校验数据库与引用文件后升级到 Schema 2；旧备份没有文件校验和，会在预览中明确显示。新版库应使用 0.2.0 或更新版本打开。缺失文件、校验和不匹配、未来版本、符号链接和带 WAL/SHM 的不完整备份会拒绝恢复。校验范围上限为 100000 个文件、10 GB。
