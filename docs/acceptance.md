...
## 0.3.3 磁盘根目录安装修复（2026-09-23）

- 修复安装在 `E:\AgentValue` 等磁盘根目录下的子文件夹时，迁移旧收藏对已存在的盘根执行 `mkdir`，导致 Windows 返回 `EPERM` 的启动错误。
- 27 项数据测试通过；使用独立测试数据在 E 盘根目录迁移成功，测试目录已清理。
- 将原有收藏复制并校验到 `E:\AgentValue-data` 后，已安装的 0.3.2 可以启动并读取收藏；原始 `.agentvault` 未删除。
